...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20200331102914_SecMig', N'3.1.3');

GO

ALTER TABLE [Items] DROP CONSTRAINT [FK_Items_ITypes_ITypeId];
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Items]') AND [c].[name] = N'TypeId');
IF @var4 IS NOT NULL EXEC(N'ALTER TABLE [Items] DROP CONSTRAINT [' + @var4 + '];');
ALTER TABLE [Items] DROP COLUMN [TypeId];

GO

DROP INDEX [IX_Items_ITypeId] ON [Items];
DECLARE @var5 sysname;
SELECT @var5 = [d].[name]
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Items]') AND [c].[name] = N'ITypeId');
IF @var5 IS NOT NULL EXEC(N'ALTER TABLE [Items] DROP CONSTRAINT [' + @var5 + '];');
ALTER TABLE [Items] ALTER COLUMN [ITypeId] int NOT NULL;
CREATE INDEX [IX_Items_ITypeId] ON [Items] ([ITypeId]);

GO

ALTER TABLE [Items] ADD CONSTRAINT [FK_Items_ITypes_ITypeId] FOREIGN KEY ([ITypeId]) REFERENCES [ITypes] ([ITypeId]) ON DELETE CASCADE;

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20200407045556_Create table FMigration', N'3.1.3');
